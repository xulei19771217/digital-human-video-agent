...
export interface CaptionDependencies {
  ...
  probeAudioDuration(path: string): Promise<number>;
}
...
export function validateCaptionCues(
  ...
  if (cues.length === 0) throw new Error("Caption file contains no cues");
  let previousEnd = 0;
...
async function defaultProbeAudioDuration(path: string): Promise<number> {
...
export class CaptionProvider implements StageProvider<CaptionInput> {
  readonly stage = "captions" as const;
  readonly paid = false;

  constructor(
    private readonly dependencies: CaptionDependencies = {
      run: runProcess,
  ...
  async execute(
    ...
    validateCaptionCues(
      cues,
      await this.dependencies.probeAudioDuration(input.audioPath),
    );
    return {
      artifacts: [
        {
