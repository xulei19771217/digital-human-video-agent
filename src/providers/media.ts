...
async function defaultSearchPexels(
  query: string,
  apiKey: string,
): Promise<PexelsCandidate[]> {
  ...
  return parsed.videos.flatMap((video) => {
    const file = video.video_files
      ...
      .sort(
        (left, right) =>
          Math.abs((left.width ?? 0) - 720) -
          Math.abs((right.width ?? 0) - 720),
      )[0];
    if (!file || file.width === null || file.height === null) return [];
    return [
      {
        id: String(video.id),
        width: file.width,
...
async function localFiles(localDir: string): Promise<string[]> {
  try {
...
export class MediaProvider implements StageProvider<MediaInput> {
  readonly stage = "media" as const;
  readonly paid = false;
  ...
  async inputHash(input: MediaInput): Promise<string> {
  ...
  async resolve(input: MediaInput): Promise<MediaManifest> {
      ...
      const items = await Promise.all(
        files.map(async (sourcePath) => {
          ...
          return {
            ...
            licenseNote:
              "User-provided local media; user is responsible for rights",
            accessedAt,
    ...
    if (input.pexelsEnabled && input.pexelsApiKey) {
      try {
        for (const query of input.queries) {
          const candidates = await this.dependencies.searchPexels(
            query,
            input.pexelsApiKey,
          );
          const selected = candidates.find(
            (candidate) => candidate.height >= candidate.width,
          );
          if (!selected) continue;
          const video = await this.dependencies.download(selected.downloadUrl);
          if (video.byteLength === 0) continue;
          const localPath = join(
            input.outputDir,
            `pexels-${selected.id}.mp4`,
          );
            ...
            items: [
              {
                ...
                licenseNote:
                  "Pexels license; verify current terms before publishing",
                accessedAt,
                sha256: await sha256File(localPath),
              },
            ],
    ...
    return {
      artifacts: [
        ...manifest.items.map((item) => ({
          ...
          sha256: item.sha256,
          mediaType:
            extname(item.localPath).toLowerCase() === ".mp4"
              ? "video/mp4"
              : "image",
        })),
        {
