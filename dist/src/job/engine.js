...
export function invalidateFromStage(job, first) {
...
export function invalidateForChange(job, change) {
    return invalidateFromStage(job, change === "script" ? "voice" : "package");
}
function completedRecord(current, inputHash, result) {
    return {
        status: "completed",
        paid: current.paid,
        inputHash,
...
export class JobEngine {
    ...
    async persistProviderTaskId(jobId, stage, providerTaskId) {
        const job = await this.store.load(jobId);
        job.stages[stage].providerTaskId = providerTaskId;
        await this.store.save(job);
    }
    async runStage(original, provider, input) {
        ...
        if ((current.status === "unknown" || current.status === "failed") &&
            ...
            provider.recover) {
            const result = await provider.recover(current.providerTaskId, input, { jobId: job.id, runDir: this.store.runDir(job.id) });
            job.stages[provider.stage] = completedRecord(current, inputHash, result);
            await this.store.save(job);
            return job;
        }
        if (current.status === "unknown") {
            throw new Error("Paid stage outcome is unknown; manual recovery is required");
        }
        ...
        try {
            ...
            job.stages[provider.stage] = completedRecord(job.stages[provider.stage], inputHash, result);
            await this.store.save(job);
            return job;
        }
        catch (error) {
