...
import { createDefaultDoctorDependencies, runDoctor, } from "./commands/doctor.js";
import { createDefaultSetupDependencies, runSetup, } from "./commands/setup.js";
import { CliError, ExitCode, resumeVideoJob, runVideoJob, } from "./commands/run.js";
...
export async function main(argv) {
    let commandExitCode = ExitCode.ok;
    ...
    program
        ...
        .action(async () => {
        const profile = await runSetup(await currentCredentials(), createDefaultSetupDependencies());
        process.stdout.write(`${JSON.stringify({ configured: true, profile }, null, 2)}\n`);
    });
    program
        .command("doctor")
        ...
        const report = await runDoctor(await currentCredentials(), createDefaultDoctorDependencies());
        if (options.json) {
        ...
        if (!report.ready)
            commandExitCode = ExitCode.notConfigured;
    });
    program
        .command("run")
        ...
        .option("--speed <number>", "Narration speed from 0.5 through 2.0", Number)
    ...
    program
        ...
        process.stdout.write(`${JSON.stringify(await runStatus(jobId, options.runsDir), null, 2)}\n`);
    });
    program
        .command("resume")
        .argument("<job-id>")
