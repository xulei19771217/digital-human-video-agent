...
import { ProfileSchema, saveProfile as writeProfile, } from "../config/profile.js";
export const SIGNUP_URLS = {
    fish: "https://fish.audio/app/api-keys/",
    heygen: "https://app.heygen.com/settings",
};
export async function runSetup(existing, dependencies) {
    ...
    if (voices.length === 0)
        throw new Error("Fish Audio returned no voices");
    ...
    if (avatars.length === 0)
        throw new Error("HeyGen returned no avatars");
    const voiceId = await dependencies.select("Default Fish Audio voice", voices.map((voice) => ({ value: voice.id, label: voice.title })));
    const avatarId = await dependencies.select("Default HeyGen avatar", avatars.map((avatar) => ({ value: avatar.id, label: avatar.name })));
...
export function createDefaultSetupDependencies() {
    ...
    return {
        ...
        listFishVoices: async (apiKey) => {
            const parsed = FishVoicesSchema.parse(await fetchJson("https://api.fish.audio/model?page_size=50&page_number=1", { Authorization: `Bearer ${apiKey}` }));
        ...
        listHeygenAvatars: async (apiKey) => {
            const parsed = HeygenAvatarsSchema.parse(await fetchJson("https://api.heygen.com/v2/avatars", {
                "X-Api-Key": apiKey,
            }));
        ...
        promptOutputDir: async () => {
            const answer = await readline.question(`Output directory [${paths.runsDir}]: `);
