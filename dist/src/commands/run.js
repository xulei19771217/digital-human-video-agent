...
import { CaptionProvider } from "../providers/captions.js";
import { FishAudioProvider } from "../providers/fish-audio.js";
import { HeyGenProvider } from "../providers/heygen.js";
import { HyperFramesProvider, } from "../providers/hyperframes.js";
import { MediaProvider, } from "../providers/media.js";
import { MockAvatarProvider, MockCaptionProvider, MockVoiceProvider, } from "../providers/mock.js";
import { parseScript, PublishPackProvider, } from "../providers/publish-pack.js";
...
async function verifyCachedArtifacts(original, store) {
...
async function loadRuntimeCredentials() {
...
async function requireProfile() {
    try {
        ...
        if (error.code === "ENOENT") {
            throw new CliError("Configuration is missing. Run video-agent setup first.", ExitCode.notConfigured);
        }
        throw error;
    }
}
function createProviders(job, store, script, credentials) {
    ...
    if (!credentials.fishAudioApiKey || !credentials.heygenApiKey) {
        throw new CliError("Fish Audio and HeyGen credentials are required. Run video-agent setup.", ExitCode.notConfigured);
    }
    return {
        voice: new FishAudioProvider(credentials.fishAudioApiKey),
        avatar: new HeyGenProvider(credentials.heygenApiKey, async (providerTaskId) => await engine.persistProviderTaskId(job.id, "avatar", providerTaskId)),
        media: new MediaProvider(),
...
async function executeJob(initialJob, store, script, credentials) {
    ...
    const mediaDir = job.parameters.mediaDir ?? join(dirname(job.scriptPath), "media");
    ...
    job = await engine.runStage(job, providers.package, {
        ...
        coverPath,
        durationSeconds,
        coverTimeSeconds: Math.min(script.coverTimeSeconds, Math.max(0, durationSeconds - 0.1)),
        hook: script.hook,
        facts: script.facts,
    });
    ...
    const artifacts = Object.fromEntries(await Promise.all(outputs.map(async (path) => [
        path.split(/[\\/]/).at(-1),
        { path, sha256: await sha256File(path) },
    ])));
    const reportPath = join(outputDir, "run-report.json");
    await writeFile(reportPath, `${JSON.stringify(redact({
        schemaVersion: 1,
        jobId: job.id,
        status: "completed",
        profile: job.profile,
...
export async function runVideoJob(scriptPath, options) {
    ...
    const runsDir = resolve(options.outputDir ??
        savedProfile?.outputDir ??
        join(process.cwd(), "runs"));
    const mediaDir = resolve(options.mediaDir ?? join(dirname(absoluteScriptPath), "media"));
    ...
    if (!mock) {
        ...
        await new HeyGenProvider(credentials.heygenApiKey, async () => undefined).validate();
    }
    const job = await store.create(absoluteScriptPath, await sha256File(absoluteScriptPath), profile, {
...
export async function resumeVideoJob(jobId, runsDir) {
    const profile = await requireProfile().catch((error) => {
        if (runsDir)
            return undefined;
        throw error;
    });
    const root = resolve(runsDir ?? profile?.outputDir ?? currentAppPaths().runsDir);
