...
export async function parseScript(path) {
    ...
    return {
        ...
        mediaQueries: frontMatter.media_queries.length > 0
            ? frontMatter.media_queries
            : [frontMatter.title],
        topics: frontMatter.topics.length > 0
            ? frontMatter.topics
            : ["非洲旅行", "旅行知识"],
    };
}
function packageTemplateDir() {
    ...
    if (!found)
        throw new Error("Publishing templates are missing");
    return found;
}
...
export class PublishPackProvider {
    ...
    stage = "publish";
    paid = false;
    constructor(templateDir = packageTemplateDir()) {
    ...
    async execute(input, _context) {
        ...
        for (const [templateName, outputName] of targets) {
            const template = await readFile(join(this.templateDir, templateName), "utf8");
