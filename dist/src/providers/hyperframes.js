...
function findPackagePath(relative) {
    ...
    if (!found)
        throw new Error(`Package asset not found: ${relative}`);
    return found;
}
async function defaultProbeMaster(path) {
...
async function defaultCaptureCover(masterPath, outputPath, atSeconds) {
...
export function createDefaultHyperFramesDependencies() {
    return {
        run: runProcess,
        templateDir: findPackagePath(join("templates", "hyperframes")),
        gsapPath: findPackagePath(join("node_modules", "gsap", "dist", "gsap.min.js")),
...
function buildFactClips(facts, duration) {
    ...
    facts.forEach((fact, index) => {
        ...
        animations.push(`timeline.fromTo("#${id}-inner", { opacity: 0, y: 24 }, { opacity: 1, y: 0, duration: ${seconds(Math.min(0.4, clipDuration / 3))}, ease: "power2.out" }, ${seconds(start)});`);
    });
    return { markup: markup.join("\n"), animations: animations.join("\n") };
}
function buildCaptionClips(captionsSource, duration) {
    const markup = [];
    const animations = [];
    for (const cue of parseSrt(captionsSource)) {
        ...
        animations.push(`timeline.fromTo("#${id}-text", { opacity: 0, y: 12 }, { opacity: 1, y: 0, duration: ${seconds(Math.min(0.18, (end - start) / 3))}, ease: "power1.out" }, ${seconds(start)});`);
    }
    return { markup: markup.join("\n"), animations: animations.join("\n") };
}
async function buildMediaClips(manifest, projectDir, duration) {
    const markup = [];
...
export class HyperFramesProvider {
    ...
    stage = "package";
    paid = false;
    constructor(dependencies = createDefaultHyperFramesDependencies()) {
    ...
    async execute(input, _context) {
        ...
        await copyFile(this.dependencies.gsapPath, join(input.projectDir, "vendor", "gsap.min.js"));
        const captionsSource = await readFile(input.captionsPath, "utf8");
        const facts = buildFactClips(input.facts, input.durationSeconds);
        const captions = buildCaptionClips(captionsSource, input.durationSeconds);
        const media = await buildMediaClips(input.media, input.projectDir, input.durationSeconds);
        ...
        const html = template
            ...
            .replace("__ANIMATIONS__", [hookAnimation, facts.animations, captions.animations].join("\n"));
        await writeFile(templatePath, html, "utf8");
        await writeFile(join(input.projectDir, "episode.json"), `${JSON.stringify({
            schemaVersion: 1,
            hook: input.hook,
            facts: input.facts,
            durationSeconds: input.durationSeconds,
        ...
        const renderResult = await this.dependencies.run(render.command, render.args);
        ...
        await this.dependencies.captureCover(input.outputPath, input.coverPath, input.coverTimeSeconds);
        return {
            artifacts: [
                {
